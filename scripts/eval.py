"""A lightly adapted version of the following CEAF-REE scoring script:

https://github.com/xinyadu/gtt/blob/master/eval.py

An effort was made to keep this script as close to the original as possible,
which means that it will run *very* slowly in cases where the predictions
contain many predicted templates for a particular example. This is because
the original script uses a brute-force approach to finding the best mapping
between predicted and gold templates. For all model predictions included in
this repo, the script finishes in at most a few minutes, but more typically
within seconds.
"""
import re
import string
                ...
                for idx in range(len(preds[docid][idx_temp][role])):
                    for idy in range(len(preds[docid][idx_temp][role][idx])):
                        preds[docid][idx_temp][role][idx][idy] = normalize_string(
                            preds[docid][idx_temp][role][idx][idy], is_chinese
                        )
    for docid in golds:
                ...
                for idx in range(len(golds[docid][idx_temp][role])):
                    for idy in range(len(golds[docid][idx_temp][role][idx])):
                        golds[docid][idx_temp][role][idx][idy] = normalize_string(
                            golds[docid][idx_temp][role][idx][idy], is_chinese
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="set this flag if scoring Chinese data to normalize mentions",
    )
